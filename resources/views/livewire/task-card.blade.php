...
<x-card-form>
    ...
    <div wire:init="ready" class="position-relative" style="min-height: 55px; overflow: hidden;">
            ...
            <div class="d-flex justify-content-between align-items-start">
                ...
                <div class="d-flex gap-1 ms-auto">
                    ...
                    <button class="btn btn-sm"
                            @click="
                                if (!detailsOpen) {
                                  // Closed -> open details and open edit
                                  detailsOpen = true;
                                  requestAnimationFrame(()=>{ editOpen = true; });
                                } else if (!editOpen) {
                                  // Details open, edit closed -> open edit
                                  editOpen = true;
                                } else {
                                  // Details open, edit open -> close edit AND close details
                                  editOpen = false;
                                  detailsOpen = false;
                                }
                            "
                    ...
                    @livewire('delete-task-button', ['task' => $task], key('delete-task-' . $task->id . '-' . $ts))
            ...
            <div x-show="detailsOpen" x-cloak x-transition.duration.150ms class="mt-2">
                ...
                <div x-show="!editOpen" x-cloak x-transition.duration.150ms>
                    ...
                    @if ($task->result_text || $task->result_file)
                        ...
                        @endif
                        @if ($task->result_file)
                            <a href="{{ \Storage::url($task->result_file) }}"
                    ...
                    {{-- Livewire Action Buttons (Author only) --}}
