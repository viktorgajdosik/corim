<form class="custom-floating-label" enctype="multipart/form-data"
      wire:poll.10s="refreshParticipants"
      x-on:submit.prevent="
...
>
  ...
  {{-- Assign User (same look as before) --}}
  <div class="form-floating mb-3">
    <select wire:model.defer="assigned_user_id" class="form-select bg-dark text-white @error('assigned_user_id') is-invalid @enderror" id="assigned_user_id">
      <option value="" disabled {{ $assigned_user_id ? '' : 'selected' }}>Choose participant</option>
      @foreach ($participants as $applicant)
        <option value="{{ $applicant->user->id }}" wire:key="opt-update-{{ $applicant->user->id }}">
    ...
    <button type="submit"
      ...
      <template x-if="$store.ui.updateLoading[{{ $task->id }}]">
        <span class="d-inline-flex align-items-center gap-1">
          Updating <span class="spinner-grow spinner-grow-sm align-middle"></span>
        </span>
      </template>
    </button>
